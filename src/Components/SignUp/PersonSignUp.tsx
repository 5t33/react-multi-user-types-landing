...
const PersonSignUp: React.FC<PersonSignUpProps> = (
  props: PersonSignUpProps
) => {
  const { initialValues } = props;
  const signUpState: SignUpStateType = useSignUpState();
  const signUpDispatch: SignUpDispatch = useSignUpDispatch();
  const confirmationOnClick = () =>
    signUpDispatch({
      type: SignUpActionTypeEnum.SET_STEP,
      payload: SignUpStepEnum.CONFIRMATION_STEP
    });
  return (
    <Formik
