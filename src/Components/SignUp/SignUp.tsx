...
const SignUpForm: React.FC<SignUpFormProps> = (props: SignUpFormProps) => {
  ...
  const navBack = () =>
    signUpDispatch({ type: SignUpActionTypeEnum.DECREMENT_STEP });

  const isPerson =
    signUpState.userType === UserTypes.Artist ||
    signUpState.userType === UserTypes.Fan;
  ...
  return (
    <div className={classes.root}>
      {showInnerState && (
        <>
          <NavButton
            className={classes.LocalNavButton}
            direction={"left"}
            onClick={navBack}
          />
          ...
          {isPerson && (
            <PersonSignUp
              initialValues={
                get(
                  [`${signUpState.userType}`, "basicInfoData"],
                  signUpState
                ) || {}
              }
            />
      ...
      {!showInnerState &&
        signUpState.outerStep === SignUpStepEnum.INITIAL_STEP && (
        <IntitialSignUp />
      )}

      {!showInnerState &&
        signUpState.outerStep === SignUpStepEnum.CONFIRMATION_STEP && (
        <SignUpSuccess />
      )}
    </div>
  );
