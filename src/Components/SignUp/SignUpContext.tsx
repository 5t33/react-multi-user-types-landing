...
const SignUpStateContext = React.createContext<SignUpStateType>(
  initialSignUpState
);
const SignUpDispatchContext = React.createContext<SignUpDispatch>(() => {
  throw new Error(DispatchPreUse);
});
...
const signUpReducer = (state: SignUpStateType, action: SignUpAction) => {
  ...
  const firstStep = 0;
  const step = immutable.getIn(
    state,
    ["outerStep"],
    SignUpStepEnum.INITIAL_STEP
  );
  const innerStep = immutable.getIn(
    state,
    [`${state.userType}`, "innerStep"],
    firstStep
  );
  const innerStepList = userStepListMap[userType];
  const lastInnerStep = innerStepList.slice(-1)[0]; // eslint-disable-line no-magic-numbers
  const firstInnerStep = innerStepList[0]; // eslint-disable-line no-magic-numbers
  switch (action.type) {
    ...
    case SignUpActionTypeEnum.INCREMENT_STEP:
      if (step === SignUpStepEnum.INNER_STEPS && innerStep < lastInnerStep) {
        return immutable.setIn(
          state,
          [`${state.userType}`, "innerStep"],
          state[`${state.userType}`].innerStep + stepIncrement
        );
      } else {
        return immutable.setIn(
          state,
          ["outerStep"],
          state.outerStep + stepIncrement
        );
      }
    case SignUpActionTypeEnum.DECREMENT_STEP:
      if (
        step > SignUpStepEnum.INNER_STEPS ||
        (step === SignUpStepEnum.INNER_STEPS && innerStep === firstInnerStep)
      ) {
        return immutable.setIn(
          state,
          ["outerStep"],
    ...
    default:
      throw new Error(
        ReducerUnrecognizedActionError(
          (action.type as SignUpStepEnum).toString()
        )
      );
  }
};
